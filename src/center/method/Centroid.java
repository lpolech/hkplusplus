...
public class Centroid implements CenterMethod{
	...
	public Cluster makeCluster(Data points, Measure measure)
	{
		...
		return new Cluster(points.getPoints(), new DataPoint(centroid, null, "centroid"), ColorPalette.getNextColor(), rootId, rootId);
	}

	@Override
	public Cluster updateCenter(Cluster cluster, Measure measure) {
		double[] newCoordinates = measure.updateCenter(cluster.getPoints());
		cluster.setCenter(new DataPoint(newCoordinates, null, cluster.getCenter().getClassAttribute()));
		return cluster;
	}
}
